...
<div class="page">
    [ <a href="<app:pagingUtil url="${requestURI}" pageNum="${sc.pageNo-1}"/>">首页</a> 
    	<a href="<c:if test="${sc.pageNo-1 >= 1 }"><app:pagingUtil url="${requestURI}" pageNum="${sc.pageNo-1}"/></c:if><c:if test="${sc.pageNo-1 < 1 }">javascript:void(0)</c:if>">上一页</a> 
    	<a href="<c:if test="${sc.pageNo+1 <= sc.pageCount }"><app:pagingUtil url="${requestURI}" pageNum="${sc.pageNo+1}"/></c:if><c:if test="${sc.pageNo+1 > sc.pageCount }">javascript:void(0)</c:if>">下一页</a> 
    	<a href="<app:pagingUtil url="${requestURI}" pageNum="${sc.pageCount}"/>">尾页</a>
    ] 
    [ 首页:${sc.pageNo }/${sc.pageCount }页 ] [ 每页显示${sc.rows }条记录 / 总共${sc.totalCount }条记录 ]
</div>