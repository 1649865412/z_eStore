...
<cartmatic:img url="<%=sku.getImage()%>" url2="<%=sku.getImage2() %>" mediaType="<%=mediaType%>" size="${size}" alt="" height="${height}" width="${width}" isUrlOnly="${isUrlOnly}" id="${id}" mouseOverHandler="${mouseOverHandler}" onClickHandler="${onClickHandler}" onLoadHandler="${onLoadHandler}" isLazyload="${isLazyload}" isList="${isList}"></cartmatic:img>
