...
<%@ attribute name="showType"	type="java.lang.String"%>
<c:choose>
		...
		<c:choose>
			<c:when test="${not empty ad.redirectUrl}">
				<c:if test="${showType=='1'}">
					<li style="background: url(<cartmatic:img isUrlOnly="true" url="${ad.url}" mediaType="a_and_d" width="${ad.adPositionType.width}" height="${ad.adPositionType.height}" alt="${ad.advertisementDetail}"></cartmatic:img>) 50% 0 no-repeat;">
				...
				<c:if test="${showType=='2'}">
					...
					<p style="margin-top: 6px; line-height: 16px;">
						${ad.advertisementDetail }
					</p>
				</c:if>
			</c:when>
			<c:otherwise>
				<c:if test="${showType=='1'}">
					<li style="background: url(<cartmatic:img isUrlOnly="true" url="${ad.url}" mediaType="a_and_d" width="${ad.adPositionType.width}" height="${ad.adPositionType.height}" alt="${ad.advertisementDetail}"></cartmatic:img>) 50% 0 no-repeat;"></li>
				</c:if>
				<c:if test="${showType=='2'}">
					<p>
						<cartmatic:img url="${ad.url}" mediaType="a_and_d" width="${ad.adPositionType.width}" height="${ad.adPositionType.height}" alt="${ad.advertisementDetail}"></cartmatic:img>
